...
public class FacebookRequester {
	...
	public List<FbUser> getPhotos() throws FacebookError, JSONException {
		Bundle b = new Bundle();
		b.putString("limit", "10");
		
        String url = "https://graph.facebook.com/" + mAccount.getId()
                + "/albums&access_token="
                + mFacebook.getAccessToken();
		
		String response = null;
		try {
			response = mFacebook.request(url, b);
		} catch (IOException e) {
			e.printStackTrace();
		}
